...
void main() {
  ...
  group('ChatPage Tests for tablet', () {
    testWidgets(
        'Displays no chat messages on ChatPage for tablet when no chats and groups exist',
    ...
    testWidgets("Group chat page renders correctly when a group is selected",
        (WidgetTester tester) async {
      tester.view.physicalSize = const Size(1194.0, 834.0);
      tester.view.devicePixelRatio = 1.0;
      AuthService.setUid('user1');
    ...
    testWidgets(
      ...
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
            groupProvider.overrideWith(
              (ref, groupId) => groupId == '123' ? fakeGroup1 : fakeGroup2,
            ),
            userProvider.overrideWith(
              (ref, uid) async => mockDatabaseService.getUserData(uid),
            ),
        ...
        ProviderScope(
          ...
          child: CupertinoApp(
            home: ChatTabletPage(
              ...
              imagePicker: mockImagePicker,
              selectedGroup: null,
              selectedPrivateChat: fakePrivateChat1,
              selectedUser: UserData.fromSnapshot(documentSnapshot2),
              eventService: mockEventService,
            ),
          ),
        ),
    ...
    group('Test chat functionality', () {
      ...
      testWidgets("Group chat page navigations work correctly for tablet",
        ...
        when(mockDatabaseService.getUnreadMessages(any, any)).thenAnswer(
          (_) => Stream.value(0),
        );
        when(mockDatabaseService.getUserDataFromUID('user1'))
            .thenAnswer((_) => Stream.value(documentSnapshot1));
